...
<body>
<div class="page-wrap">
    <form class="post-form" method="post" action="<?php echo site_url('admin/post/edit');?>">
        ...
        <div class="post-body clearfix">
            <div class="post-main-left">
                <div class="controls-row mb10">
                    <div class="pull-left">
                        ...
                        <input class="span7 mr10 input-post-title" type="text" name="title" id="J_title" value="<?php echo $info['title'];?>">
                    </div>
                    <div class="pull-right">
                        <label class="control-label-price" for="price">价格：</label>
                        <input class="span3" type="text" name="price" id="price" value="<?php echo $info['price'];?>">
                    </div>
                </div>
                ...
                <div class="form-horizontal mt20">
                    <div class="control-group">
                        ...
                        <div id="J_linkdiv" class="controls controls-row">
                            <?php foreach ($info['links'] as $key => $_link) :?>
                            <div class="J_link pull-left mb10">
                                <select class="input-medium pull-left mr10" name="link_mid[]">
                                    <?php foreach($mall_list as $val):?>
                                        <option value="<?php echo $val['id'];?>" <?php if ($val['id'] == $_link['mid']) :?>selected<?php endif;?>><?php echo $val['name'];?></option>
                                    <?php endforeach;?>
                                </select>
                                <input class="span2" type="text" name="link_title[]" placeholder="链接标题" value="<?php echo $_link['title'];?>">
                                <input class="span4" type="text" name="link_url[]" placeholder="链接地址" value="<?php echo $_link['url'];?>">
                                <?php if ($key > 0) :?>
                                <button class="J_dellink btn btn-small ml10" type="button"><i class="icon-minus-sign"></i> 删除</button>
                                <? endif;?>
                            </div>
                            <?php endforeach;?>
                    ...
                    <div class="control-group">
                        ...
                        <div class="controls">
                            <input name="seo_keywords" class="span8" type="text" id="seo_keywords" value="<?php echo $info['seo_keywords'];?>">
                        </div>
                    </div>
                    <div class="control-group">
                        <label class="control-label" for="seo_description">SEO描述文字：</label>
                        <div class="controls">
                            <textarea name="seo_description" class="span8" rows="3" id="seo_description"><?php echo $info['seo_description'];?></textarea>
                        </div>
                    </div>
                </div>
            </div>
            <div class="post-main-right">
                <div class="post-block">
                    ...
                    <div class="inside">
                        <?php if ($info['img']) :?>
                        <div class="text-center mb10" id="J_preview"><img src="<?php if ($info['img']) echo base_url('data/upload/post/cover/'.$info['img']);?>" /></div>
                        <?php else :?>
                        ...
                        <input type="hidden" name="img" id="J_img" value="<?php echo $info['img'];?>">
                    </div>
                </div>
                <div class="post-block">
                    <h3>选择分类</h3>
                ...
                <div class="post-block">
                    ...
                    <div class="inside">
                        <div id="J_tagsdiv" class="post-tag">
                            <input name="tags" class="J_tags" type="hidden" name="tags" value="<?php echo $info['tags'];?>">
                ...
                <div class="post-block">
                    ...
                    <div class="inside">
                        ...
                        <input name="author" class="input-medium" type="text" id="author" value="<?php echo $info['author'];?>">
                        <label class="control-label" for="post_time">发布时间：</label>
                        <div id="J_post_time" class="input-append">
                            <input name="post_time" class="input-medium" data-format="yyyy-MM-dd hh:mm" type="text" value="<?php echo date('Y-m-d H:i', $info['post_time']);?>">
                            <span class="add-on">
                                <i data-time-icon="icon-time" data-date-icon="icon-calendar"></i>
                            </span>
                        </div>
                        <label class="control-label" for="orderid">排序：</label>
                        <input class="input-medium" type="number" id="orderid" name="orderid" min="1" max="255" value="<?php echo $info['orderid'];?>">
                        <label class="control-label" for="author">属性：</label>
                        <label class="checkbox inline">
                            <input type="checkbox" name="topped" value="1" <?php if ($info['topped']) :?>checked<?php endif;?>> 置顶
                        </label>
                        <label class="checkbox inline">
                            <input type="checkbox" name="isrcd" value="1" <?php if ($info['isrcd']) :?>checked<?php endif;?>> 推荐
                        </label>
                        <label class="checkbox inline">
                            <input type="checkbox" name="ishot" value="1" <?php if ($info['ishot']) :?>checked<?php endif;?>> 热门
                        </label>
                    </div>
                </div>
            </div>
